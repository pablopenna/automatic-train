class_name StashArea extends Node2D

signal on_place
signal on_take

@onready var STASH_DIMENSION: int = 16

@export var stash_size: int = 12
@export var stash_per_row: int = 4
...
func _ready():
	for i in range(0, stash_size):
		...
		var slot = StashSlot.instantiate()
